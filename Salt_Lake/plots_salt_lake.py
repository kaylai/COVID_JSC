...
confirmed_data = salt_lake_data["Confirmed"].tolist()
daily_confirmed = [0]
...
salt_lake_data["Daily_Confirmed"] = daily_confirmed

# determine outliers
q_low = salt_lake_data["Daily_Confirmed"].quantile(0.01)
q_hi  = salt_lake_data["Daily_Confirmed"].quantile(0.99)
df_filtered = salt_lake_data[(salt_lake_data["Daily_Confirmed"] < q_hi) & (salt_lake_data["Daily_Confirmed"] > q_low)]

#Calculate whether negative or postive trend in last 14 days
date_list = salt_lake_data["Date"].tolist()
...
salt_lake_data["Confirmed_Moving"] = confirmed_moving
...
salt_lake_data["Confirmed_Cum_Moving"] = confirmed_cum_moving

#DO SOME EPLOTTING
fig, ax = plt.subplots(2, figsize=(10,10))
barpoints = ax[0].bar(datetime_list, salt_lake_data["Daily_Confirmed"], color='papayawhip', label="Daily Confirmed Cases")
trend_barpoints = ax[0].bar(last_14_days_datetimes, last_14_days, color=trend_color, label="Last 14 Days")
confirmed_line = ax[0].plot(datetime_list, salt_lake_data["Confirmed_Moving"], '-', linewidth=3, color='darkorange', label="5-day moving average")
ax[0].axvspan(closest_date_with_data, datetime_list[-1], alpha=0.25, color=trend_color)
cum_barpoints = ax[1].bar(datetime_list, salt_lake_data["Confirmed"], color='lightblue', label="Cumulative Confirmed Cases")
ax[1].plot(datetime_list, salt_lake_data["Confirmed_Cum_Moving"], '-', linewidth=3, color='darkblue', label="5-day moving average")
fig.autofmt_xdate()
ax[1].fmt_xdata = mdates.DateFormatter('%m-%d-%Y')
ax[1].xaxis.set_major_locator(mdates.AutoDateLocator())
# set y-max without outliers
if df_filtered.Daily_Confirmed.max() > salt_lake_data.Confirmed_Moving.max():
	my_y_max = df_filtered.Daily_Confirmed.max()
else:
	my_y_max = salt_lake_data.Confirmed_Moving.max() + 100
