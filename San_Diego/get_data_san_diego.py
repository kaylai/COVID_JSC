...
datetimerange = pd.date_range(start='02/06/2020', end=datetime.today()).to_pydatetime().tolist()
daterange = []
...
for date in daterange:
	...
	try:
		...
		maricopa = pd.DataFrame(full_csv.loc["San Diego, California, US"])
	...
	try:
		...
		maricopa = pd.DataFrame(full_csv.loc["San Diego County, CA"])
	...
	try:
		...
		maricopa = pd.DataFrame(full_csv.loc["San Diego"])
...
with pd.ExcelWriter("San_Diego_Data.xlsx") as writer:
	maricopa_data.to_excel(writer, 'San Diego Co., CA')
