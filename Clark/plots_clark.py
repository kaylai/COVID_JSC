...
confirmed_data = clark_data["Confirmed"].tolist()
daily_confirmed = [0]
...
clark_data["Daily_Confirmed"] = daily_confirmed

# determine outliers
q_low = clark_data["Daily_Confirmed"].quantile(0.01)
q_hi  = clark_data["Daily_Confirmed"].quantile(0.99)
df_filtered = clark_data[(clark_data["Daily_Confirmed"] < q_hi) & (clark_data["Daily_Confirmed"] > q_low)]

#Calculate whether negative or postive trend in last 14 days
date_list = clark_data["Date"].tolist()
...
clark_data["Confirmed_Moving"] = confirmed_moving
...
clark_data["Confirmed_Cum_Moving"] = confirmed_cum_moving

#DO SOME EPLOTTING
fig, ax = plt.subplots(2, figsize=(10,10))
barpoints = ax[0].bar(datetime_list, clark_data["Daily_Confirmed"], color='papayawhip', label="Daily Confirmed Cases")
trend_barpoints = ax[0].bar(last_14_days_datetimes, last_14_days, color=trend_color, label="Last 14 Days")
confirmed_line = ax[0].plot(datetime_list, clark_data["Confirmed_Moving"], '-', linewidth=3, color='darkorange', label="5-day moving average")
ax[0].axvspan(closest_date_with_data, datetime_list[-1], alpha=0.25, color=trend_color)
cum_barpoints = ax[1].bar(datetime_list, clark_data["Confirmed"], color='lightblue', label="Cumulative Confirmed Cases")
ax[1].plot(datetime_list, clark_data["Confirmed_Cum_Moving"], '-', linewidth=3, color='darkblue', label="5-day moving average")
fig.autofmt_xdate()
ax[1].fmt_xdata = mdates.DateFormatter('%m-%d-%Y')
ax[1].xaxis.set_major_locator(mdates.AutoDateLocator())
# use filtered dataset to get y max (no outliers)
if df_filtered.Daily_Confirmed.max() > clark_data.Confirmed_Moving.max():
	my_y_max = df_filtered.Daily_Confirmed.max()
else:
	my_y_max = clark_data.Confirmed_Moving.max() + 100
...
mpld3.save_html(fig, "uploads/core/templates/core/plotclark.html")
