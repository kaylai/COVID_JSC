...
try:
	past_data = pd.read_excel("Clark_Data.xlsx")
...
for date in daterange:
	if date in past_dates:
		...
		try:
			...
			clark = pd.DataFrame(full_csv.loc["Clark, Nevada, US"])

			clark = clark.transpose()
			try:
				confirmed.append(clark.iloc[0]["Confirmed"])
			except:
				confirmed.append(0)
			try:
				deaths.append(clark.iloc[0]["Deaths"])
			except:
				deaths.append(0)
			try:
				recovered.append(clark.iloc[0]["Recovered"])
			except:
				recovered.append(0)
			try:
				active.append(clark.iloc[0]["Active"])
			except:
				active.append(0)
		...
		try:
			...
			clark = pd.DataFrame(full_csv.loc["Clark County, NV"])
...
try:
	clark_data = past_data.copy()
except:
	clark_data = pd.DataFrame({})
for i in range(len(good_dates)):
	clark_data = clark_data.append({"Date": good_dates[i], "Confirmed": confirmed[i],
									  "Deaths": deaths[i], "Recovered": recovered[i],
									  "Active": active[i]}, ignore_index=True)

#Drop duplicates or it messes up plotting
clark_data = clark_data.drop_duplicates(subset="Date", ignore_index=True)

with pd.ExcelWriter("Clark_Data.xlsx") as writer:
	clark_data.to_excel(writer, 'Clark Co., NV')
