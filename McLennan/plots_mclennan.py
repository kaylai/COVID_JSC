...
header_name = "McLennan County, TX"

# create indiv file name for county
excel_file_name = county_name + "_Data.xlsx"
plot_name = "uploads/core/templates/core/plot" + county_name + ".html"
...
county_data = pd.read_excel(excel_file_name)
county_data["Date"] = county_data["Date"].astype('datetime64[ns]')

#Calculate daily totals instead of cumulative
confirmed_data = county_data["Confirmed"].tolist()
daily_confirmed = [0]
...
county_data["Daily_Confirmed"] = daily_confirmed

# determine outliers
q_low = county_data["Daily_Confirmed"].quantile(0.01)
q_hi  = county_data["Daily_Confirmed"].quantile(0.99)
df_filtered = county_data[(county_data["Daily_Confirmed"] < q_hi) & (county_data["Daily_Confirmed"] > q_low)]

#Calculate whether negative or postive trend in last 14 days
date_list = county_data["Date"].tolist()
...
county_data["Confirmed_Moving"] = confirmed_moving
...
county_data["Confirmed_Cum_Moving"] = confirmed_cum_moving

#DO SOME EPLOTTING
fig, ax = plt.subplots(2, figsize=(10,10))
barpoints = ax[0].bar(datetime_list, county_data["Daily_Confirmed"], color='papayawhip', label="Daily Confirmed Cases")
trend_barpoints = ax[0].bar(last_14_days_datetimes, last_14_days, color=trend_color, label="Last 14 Days")
confirmed_line = ax[0].plot(datetime_list, county_data["Confirmed_Moving"], '-', linewidth=3, color='darkorange', label="5-day moving average")
ax[0].axvspan(closest_date_with_data, datetime_list[-1], alpha=0.25, color=trend_color)
cum_barpoints = ax[1].bar(datetime_list, county_data["Confirmed"], color='lightblue', label="Cumulative Confirmed Cases")
ax[1].plot(datetime_list, county_data["Confirmed_Cum_Moving"], '-', linewidth=3, color='darkblue', label="5-day moving average")
fig.autofmt_xdate()
ax[1].fmt_xdata = mdates.DateFormatter('%m-%d-%Y')
ax[1].xaxis.set_major_locator(mdates.AutoDateLocator())
# use filtered dataset to get y max (no outliers)
if df_filtered.Daily_Confirmed.max() > county_data.Confirmed_Moving.max():
	my_y_max = df_filtered.Daily_Confirmed.max()
else:
	my_y_max = county_data.Confirmed_Moving.max() + 100
ax[0].set_ylim(0,my_y_max)
ax[1].set_xlabel('Date')
ax[0].set_ylabel('Daily Confirmed Cases')
ax[1].set_ylabel('Cumulative Confirmed Cases')
ax[0].set_title('COVID-19 Cases in ' + str(header_name))
ax[0].legend()
ax[1].legend()
