...
datetimerange = pd.date_range(start='03/17/2020', end=datetime.today()).to_pydatetime().tolist()
daterange = []
...
for date in daterange:
	...
	try:
		...
		maricopa = pd.DataFrame(full_csv.loc["Utah, Utah, US"])
...
with pd.ExcelWriter("Utah_Data.xlsx") as writer:
	maricopa_data.to_excel(writer, 'Utah Co., UT')
